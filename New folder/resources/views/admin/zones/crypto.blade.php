      ...
      <div class="row">
        <div class="col">
          <div class="content-title">
            <h1>Crypto</h1>
            <h2>Manage cryptography settings for your website</h2>
          </div>
  ...
  <p class="text-info">This setting was last changed 2 days ago</p>
          ...
          <div class="col-sm-12 col-md-3 right">
           ...
           <select  settingid="{{$zoneSetting->where('name','always_use_https')->first()->id }}"  style="width: 200px;" class="select2 changeableSetting" id="always_use_https" name="always_use_https">
                <option {{ $always_use_https == "off" ? "selected":"" }} value="off">Off</option>
                <option {{ $always_use_https == "on" ? "selected":"" }} value="on">ON</option>
...
Automatic HTTPS Rewrites helps fix mixed content by changing “http” to “https” for all resources or links on your web site that can be served with HTTPS.
  ...
  <p class="text-info">This setting was last changed 2 days ago</p>
        ...
        <?php
	if(!is_null($zoneSetting->where('name','sha1_support')->first()))
{
                        ...
                        @foreach ($customCertificates as $customCertificate)
                            <tr id="certificate_{{ $customCertificate->id }}" data-entry-id="{{ $customCertificate->id }}">
                              ...
                              <?php $n++; ?>
                                ...
                                <td>
                                  <input disabled="" class=""  record-id="{{$customCertificate->id}}"  type="checkbox" data-onstyle="success" data-offstyle="secondary" {{ $customCertificate->status == "active" ? "checked" : "" }} data-toggle="toggle" data-on="<i class='fa fa-check'></i> Active" data-off="<i class='fa fa-exclamation'></i> expired">

                               
                                  
                                    <a style="margin-left: 15px;" class="deleteCustomCertificate" certificate-id="{{$customCertificate->id}}" class="btn btn-outline-primary">

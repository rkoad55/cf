...
<div class="firewall">
    ...
    <div class="card card-2col">
      <div class="left">
        <div class="card-body"><h2 style="display: inline">Page Rules for {{ $zone->name }}</h2>

        <div class="float-right">
          
      <a 
      @if($allowed<=count($pagerules)) style="display: none;"  @endif
 class="btn btn-primary" href="#" id="addPageRuleBtn" data-toggle="modal" data-target="#rule-edit-modal" > Add New Rule</a>
                        ...
                        @foreach ($pagerules as $rule)
                            <tr id="rule_{{ $rule->id }}" data-entry-id="{{ $rule->id }}">
                              ...
                              <?php $n++; ?>
                                ...
                                <td>
                                  <input class="pageRuleStatus"  record-id="{{$rule->id}}"  type="checkbox" data-onstyle="on" data-offstyle="off" {{ $rule->status == "active" ? "checked" : "" }} data-toggle="toggle" data-on="On" data-off="Off">
                                  ...
                                  <a style="margin:20px;"  data-toggle="modal" data-target="#rule-edit-modal_{{ $rule->id }}" class="editPageRule" rule-id="{{$rule->id}}" class="btn btn-secondary">
                                    ...
                                    <a class="deletePageRule" rule-id="{{$rule->id}}" class="btn btn-outline-primary">
...
<div class="col-lg-3">
   @if($rule->pageRuleAction->first()->action != $action->action)
    <a href="#" class=" btn btn-inverse hideAction"> <i class="fas fa-times"></i></a>
   @endif
</div>
...
<div class="modal" id="rule-edit-modal" data-reveal>
   ...
   <div class="modal-dialog modal-lg" >
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">Add new Page Rule</h4>
      </div>
      <div class="modal-body">

...
class MaxCDN {
	...
	private function execute($selected_call, $method_type, $params) {
		...
		if($data->code!=200 AND $data->code!=201)
		{
			if(is_array($data->error->message))
			{

				$message=(string)$data->error->message[0];
				// $message="test";
			}
			else
			{
			...
			echo $message;
			// throw new \Exception($message, 525,null);
		}
	...
	public function get($selected_call, $params = array()){
		...
		return json_decode($this->execute($selected_call, 'GET', $params));
	}
